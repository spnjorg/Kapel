...
class Laporan extends CI_Controller{
	...
	function index(){
		...
		$this->load->view('parhaladoo/v_laporan',$x);
	}
	...
	function ina(){
		...
		$this->load->view('parhaladoo/v_ina',$x);
	}
	...
	function naposo(){
		...
		$this->load->view('parhaladoo/v_naposo',$x);
	}
	...
	function remaja(){
		...
		$this->load->view('parhaladoo/v_remaja',$x);
	}
	...
	function skm(){
		...
		$this->load->view('parhaladoo/v_skm',$x);
	}
	...
	function lansia(){
		...
		$this->load->view('parhaladoo/v_lansia',$x);
	}
	...
	function laporan_parhalado(){
		...
		foreach($dataparhalado as $id=>$data){
			$pdf->Cell(8,6,'',0,0);
			$pdf->Cell(8,6,$no,1,0,'C');
			$pdf->Cell(45,6,$data['jemaat_nama'],1,0);
			$pdf->Cell(45,6,$data['jemaat_tmptlahir'].','.$data['jemaat_tgllahir'],1,0);
			$pdf->Cell(60,6,$data['kk_alamat'].', Lk. '.$data['lingkungan_nama'],1,0);
			$pdf->Cell(60,6,$data['parhalado_jabatan'],1,0,'');
			$pdf->Cell(20,6,$data['parhalado_dilantik'],1,0,'C');
			$pdf->Cell(30,6,$data['jemaat_pendidikan'],1,0,'C');
			$pdf->Cell(35,6,$data['jemaat_pekerjaan'],1,0,'');
			$pdf->Cell(25,6,$data['jemaat_notelp'],1,1,'C');
			$no++;
		}
		$pdf->SetFont('Arial','',11);
		$pdf->Cell(15,6,'',0,1);
	...
	function laporan_dewan(){
		...
		foreach($koinonia as $id=>$data){
			...
			$pdf->Cell(35,6,$data['dewan_jabatan'],1,1,'C');
			$no++;
		}
